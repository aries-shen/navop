...
fn export_header(database: &str, exported_at: &str) -> String {
    let database = database
        ...
        .collect::<String>();
    format!("-- Database export: {database}\n-- Date: {exported_at}\n-- Generated by Navop\n\n")
}
...
impl FormatHandler for SqlFormatHandler {
    ...
    async fn export_with_progress(
        ...
        let header = export_header(&config.database, &exported_at);
        let mut output = if is_streaming {
            send_progress(ExportProgressEvent::HeaderExported { data: header });
            String::new()
        } else {
            header
        };
...
#[cfg(test)]
mod tests {
    use super::{collect_sql_statements, export_header};
    use std::io;

    #[test]
    fn export_header_contains_database_date_and_generator() {
        assert_eq!(
            export_header("comi_ai_manager", "2026-08-21 16:12:44"),
            "-- Database export: comi_ai_manager\n\
             -- Date: 2026-08-21 16:12:44\n\
             -- Generated by Navop\n\n"
        );
    }

    #[test]
    fn export_header_keeps_database_name_on_one_comment_line() {
        assert_eq!(
            export_header("primary\r\n-- injected", "2026-08-21 16:12:44"),
            "-- Database export: primary  -- injected\n\
             -- Date: 2026-08-21 16:12:44\n\
             -- Generated by Navop\n\n"
        );
    }
