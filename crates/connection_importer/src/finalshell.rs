...
pub fn preview_default_ssh_connections(
    ...
    let path = default_connections_path()
        .ok_or_else(|| ImportError::SourceDataNotFound("FinalShell connections".to_string()))?;
...
pub fn preview_ssh_connections_from_path(
    ...
    for path in json_files(path.as_ref())? {
        ...
        imported.extend(parse_finalshell_connections_json(&contents, options)?);
    }
    Ok(imported)
}

pub fn parse_finalshell_connections_json(
    contents: &str,
    options: ImportOptions,
) -> Result<Vec<ImportedSshConnection>, ImportError> {
    let root: Value = serde_json::from_str(contents)
        .map_err(|error| ImportError::InvalidSourceData(error.to_string()))?;
    connection_items(&root)
        .iter()
        .map(|item| parse_connection(item, options))
        .collect()
}
...
fn parse_connection(
    value: &Value,
    options: ImportOptions,
) -> Result<ImportedSshConnection, ImportError> {
    let host = required_string_any(value, &["host", "hostname", "ip"])?;
    let source_id = string_any(value, &["id", "uuid", "key"]).unwrap_or(host);
    Ok(ImportedSshConnection {
        source: ImportSourceKind::FinalShell,
        source_id: source_id.to_string(),
        name: string_any(value, &["name", "label", "title"])
            .unwrap_or(source_id)
            .to_string(),
        host: host.to_string(),
        port: port(value).unwrap_or(22),
        username: string_any(value, &["user_name", "username", "user"])
            .unwrap_or_default()
            .to_string(),
...
fn connection_items(root: &Value) -> Vec<&Value> {
    root.as_array()
...
fn auth_method(value: &Value) -> ImportedSshAuthMethod {
    if let Some(key_path) = string_any(
        value,
        &[
            "private_key",
            "privateKey",
...
fn candidate_roots() -> Vec<PathBuf> {
    ...
    vec![
        home.join(".finalshell/conn"),
        home.join("Library/FinalShell/conn"),
        home.join("AppData/Roaming/FinalShell/conn"),
        home.join(".config/finalshell/conn"),
    ]
}

fn has_host(value: &Value) -> bool {
    string_any(value, &["host", "hostname", "ip"]).is_some()
}
