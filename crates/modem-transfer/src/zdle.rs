// SPDX-License-Identifier: MIT OR Apache-2.0
// Copyright (c) 2017-2020 Alexey Arbuzov
// Copyright (c) 2023-2025 Jarkko Sakkinen

/// ZMODEM escape table, a constant array for escaping characters.
///
/// An entry unequal to its index means the byte is escaped as `ZDLE` followed
/// by the table entry. Like lrzsz's `zsendline_tab`, 0x7f and 0xff are sent
/// raw (entry equals index): vanilla lrzsz receivers accept raw 0x7f/0xff but
/// reject the escaped forms `18 6c` / `18 6d` ("Bad escape sequence"), so
/// escaping them would break binary uploads to stock `rz` on Linux hosts.
pub const ZDLE_TABLE: [u8; 0x100] = [
    ,
];

/// ZMODEM unescape table, a constant array for converting escaped characters back.
pub const UNZDLE_TABLE: [u8; 0x100] = [
    ,
];
