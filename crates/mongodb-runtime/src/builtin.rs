...
impl MongoConnection for MongoConnectionImpl {
    ...
    async fn find_documents(
        ...
        if collection_name == "system.sessions" {
            return self
                .find_system_sessions(database_name, filter, &sdk_find_options(&options))
                .await;
        }
        ...
        let mut cursor = collection
            .find(actual_filter)
            .with_options(sdk_find_options(&options))
            .await
            .map_err(|e| {
...
fn build_set_update_document_from_fields(set_fields: Document) -> Document {
    doc! { "$set": set_fields }
}

fn sdk_find_options(options: &MongoFindOptions) -> SdkFindOptions {
    SdkFindOptions::builder()
        .limit(options.limit)
        .skip(options.skip.map(|skip| skip.max(0) as u64))
