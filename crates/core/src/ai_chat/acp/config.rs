...
impl AcpAgentConfig {
    ...
    pub fn with_args(mut self, args: Vec<String>) -> Self {
        if let AcpTransport::Stdio { args: current, .. } = &mut self.transport {
            *current = args;
        }
        self
    }
    ...
    pub fn with_env(mut self, env: Vec<(String, String)>) -> Self {
        if let AcpTransport::Stdio { env: current, .. } = &mut self.transport {
            *current = env;
        }
        self
    }
...
mod tests {
    ...
    use super::{AcpAgentConfig, AcpStderrLevel, AcpTransport, classify_acp_stderr};
    ...
    fn strips_ansi_color_sequences_from_acp_logs() {
        let line = "\x1b[2m2026-06-09T05:48:33Z\x1b[0m \x1b[34mDEBUG\x1b[0m codex";

        assert_eq!(strip_ansi_escapes(line), "2026-06-09T05:48:33Z DEBUG codex");
    }

    #[test]
    fn constructs_http_transport_config() {
        let config = AcpAgentConfig::new_http("local", "Local ACP", "http://127.0.0.1:9876");
