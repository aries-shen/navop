...
impl PersonalSyncConflictRepository {
    ...
    pub fn list(&self, backend_profile_id: &str) -> Result<Vec<PersonalSyncConflict>> {
...
impl PersonalSyncStatusRepository {
    ...
    pub fn save(&self, status: &PersonalSyncStoredStatus) -> Result<()> {
        self.conn.execute(
    ...
    pub fn get(&self, backend_profile_id: &str) -> Result<Option<PersonalSyncStoredStatus>> {
        let mut stmt = self.conn.prepare(
