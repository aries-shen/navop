...
impl CredentialRepository {
    ...
    fn select_sql() -> &'static str {
        "SELECT id, name, kind, username, password, private_key_path,
                private_key_content, passphrase, sync_enabled, cloud_id,
...
impl Repository for CredentialRepository {
    ...
    fn insert(&self, item: &mut Self::Entity) -> Result<i64> {
        let (password, key, passphrase) = Self::encrypted_values(item)?;
        let ts = now();
            ...
            conn.execute(
                "INSERT INTO credential_entries
                 (name, kind, username, password, private_key_path, private_key_content,
                  passphrase, sync_enabled, cloud_id, last_synced_at, team_id, owner_id,
                  created_at, updated_at)
            ...
            let updated = conn.execute(
                "UPDATE credential_entries
                 SET name = ?1, kind = ?2, username = ?3, password = ?4,
                     ...
                     sync_enabled = ?8, cloud_id = ?9, last_synced_at = ?10, team_id = ?11,
                     owner_id = ?12, updated_at = ?13 WHERE id = ?14",
                rusqlite::params![
                    item.name,
                    item.kind,
                    item.username,
