import Link from "next/link";
import { AuthShell } from "@/components/auth-shell";
import { SubmitButton } from "@/components/team-form";
import { signInAction } from "@/app/[locale]/auth-actions";
import { getDictionary, isLocale, localePath, type Locale } from "@/lib/i18n";

export default async function LoginPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale: rawLocale } = await params;
  const locale: Locale = isLocale(rawLocale) ? rawLocale : "zh-CN";
  const dict = getDictionary(locale);

  return (
    <AuthShell locale={locale} title={dict.auth.loginTitle} copy={dict.home.subtitle}>
      <form className="stack" action={signInAction}>
        <input type="hidden" name="locale" value={locale} />
        <label className="stack">
          {dict.auth.email}
          <input className="input" type="email" name="email" required />
        </label>
        <label className="stack">
          {dict.auth.password}
          <input className="input" type="password" name="password" required minLength={8} />
        </label>
        <SubmitButton>{dict.auth.loginButton}</SubmitButton>
        <p className="muted">
          {dict.auth.needAccount}{" "}
          <Link href={localePath(locale, "/register")}>{dict.auth.goRegister}</Link>
        </p>
      </form>
    </AuthShell>
  );
}
