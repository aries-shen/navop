...
| 项目 | 内容 |
| --- | --- |
| 状态 | Partially Implemented（部分实施，复杂项待后续优化） |
| 日期 | 2026-08-01 |
| 来源 | GitHub Issue #30「数据库建议」 |
| 本轮实施 | #3 提交前同步右侧编辑器、#6 当前表设计器快捷键、#8 文字选区对比度 |
| 后续优化 | #1 临时查询快捷键、#2 右侧面板调整宽度、#4 MySQL 断线恢复、#5 生产环境标识与水印、#7 表设计器批量列操作、#9 Shift 连续整行选择 |
| 适用范围 | `db_view`、`db`、`one_ui`、`one-core`、cloud sync、主程序设置页 |

> 本文记录 Issue #30 的整体设计和后续拆分方向。本轮只实施低风险、边界清晰的小项；标记为“后续优化”的内容仍是设计提案，不代表仓库已经具备对应能力。
...
1. 立即完成可以复用现有事件链、无需修改持久化模型的小项；
2. 对涉及数据库重试语义、跨设备同步、复杂选区状态或跨数据库列定义转换的需求，只记录完整方案；
3. 后续按独立 PR 实施，每个 PR 具备自己的回归测试和验收范围。

## 2. Issue 需求映射

| 编号 | 需求摘要 | 本轮状态 | 主要原因 |
| --- | --- | --- | --- |
| 1 | 从数据表快捷打开临时查询并生成 `SELECT` | 后续优化 | 涉及 Tab 创建、标识符引用、数据库/schema 上下文和 SQL 方言 |
| 2 | 右侧编辑框拖动改变宽度 | 后续优化 | 需要拖动状态、尺寸约束、光标和布局回归验证 |
| 3 | 右侧编辑后可直接提交，无需先失焦 | 本轮实施 | 现有 `flush_pending` 已能同步值，只缺保存前调用 |
| 4 | MySQL 空闲断线后快速恢复 | 后续优化 | 涉及连接失效识别、重连和“哪些请求允许重试”的安全边界 |
| 5 | 生产环境标识和可选页面水印 | 后续优化 | 涉及连接持久化、云同步、表单、Tab 上下文和安全展示 |
| 6 | 从数据表快捷打开表设计器 | 本轮实施 | 现有事件链完整，只缺 action、默认绑定和设置项 |
| 7 | 表设计器按 ↓ 新增、批量复制粘贴列 | 后续优化 | 当前是单行选区，跨数据库粘贴还涉及能力降级 |
| 8 | 提升编辑框文字选区背景对比度 | 本轮实施 | 可通过默认主题 selection token 局部完成 |
| 9 | 数据表按 Shift 连续选择 | 后续优化 | 整行范围选区需统一状态、渲染、复制和刷新语义 |
...
`DataGrid` 只发出意图事件，不直接创建 Tab：

```text
DataGrid action
  -> TableDataTabEvent::OpenCurrentTableQueryRequested
  -> DbTreeView / workspace controller
  -> CreateNewQuery
  -> SqlEditorTab
```
...
表名、schema 名和 database 名必须通过：

```text
crates/db/src/plugin.rs
DatabasePlugin::quote_identifier
```

按目标数据库方言分别引用，禁止通过字符串拼接裸表名生成 SQL。限定名的层级应复用现有 plugin 能力或新增小型 helper，并覆盖 MySQL、PostgreSQL、SQL Server、SQLite 等差异。
...
当前 `CellPreviewHost` 使用固定宽度：

```text
PAGE_PREVIEW_WIDTH = 420px
```

建议在 `CellPreviewHost` 内新增：

```rust
preview_width: Pixels
```

并在面板左侧增加竖向拖动柄。实现应复用仓库已有的 drag resize 模式：

- `crates/db_view/src/database_table_columns.rs`
...
- 默认宽度：420px；
- 最小宽度：280px 或 320px；
- 最大宽度：800px，同时不得挤压左侧数据表低于 `GRID_MIN_WIDTH`；
- 拖动柄使用列调整光标；
- 拖动期间只更新局部 host 状态；
- 首版可只在当前 Tab 生命周期中保留宽度，后续再评估是否写入用户设置。

验收需覆盖窄窗口、窗口缩放、打开/关闭面板、多个 Tab 各自状态，以及面板内长文本的水平/垂直滚动。
...
普通单元格范围选择已有基础能力，但“点击行号后 Shift 点击另一个行号”不能只调用现有 `extend_selection_to`。当前还存在以下耦合：

- legacy selection 同步可能把状态切回单元格；
- 行高亮主要依赖单个 `selected_row`；
- 复制、删除和事件语义需要理解整行范围；
- 翻页、刷新、过滤和排序后 anchor 可能指向不同数据。

#### 推荐状态

在统一 selection 模型中增加 row range API，并明确维护：

```text
mode = Row
anchor_row
active_row
normalized row range
```

渲染、复制、删除和事件都从同一 selection state 读取，不再分别推断“单个选中行”和“单元格矩形”。
...
1. **PR A：右侧面板 resize（#2）**
   纯 UI 局部状态，补拖动范围测试和浅色/深色视觉验证。
2. **PR B：临时查询 action（#1）**
   完成 quoted qualified table helper 与 Tab 跳转测试。
3. **PR C：整行范围选区（#9）**
   先统一 `one_ui` selection 模型，再接入 `db_view` 的复制/删除。
...
- `one_ui` selection 状态使用纯状态单元测试覆盖正向、反向和重置；
- 数据库 SQL 生成使用各 plugin 的 identifier quoting 测试；
...
| 编号 | 状态 | 后续交付物 |
| --- | --- | --- |
| 1 | 设计完成，未实施 | 临时查询 action、SQL 引用 helper、Tab 跳转测试 |
| 2 | 设计完成，未实施 | resize handle、尺寸约束、视觉验证 |
| 3 | 本轮实施 | 保存前 flush 和失败短路 |
| 4 | 设计完成，未实施 | 错误分类、安全重试、MySQL 集成测试 |
| 5 | 设计完成，未实施 | storage/cloud sync 模型、badge、水印 |
| 6 | 本轮实施 | action、快捷键、设置页和 i18n |
| 7 | 设计完成，未实施 | 多选状态、剪贴板协议、跨方言降级 |
| 8 | 本轮实施 | 默认浅色/深色 selection token |
| 9 | 设计完成，未实施 | row-range 状态、渲染、复制与生命周期 |
