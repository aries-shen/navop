...
| Phase 5 Resource Pool UI | Planned | Implementation plan exists at `docs/superpowers/plans/2026-07-02-unified-tool-runtime-phase-5-resource-pool-ui.md`. | First checkpoint: rename context UI to resource pool/default target, keep current target selection as default-target selection, and add resource type filtering. |
