...
| Phase 6 Multi-resource execution | Done | `23cbec8 feat(agent): batch executable tool calls`, `29e6f01 feat(agent): dispatch parallel-safe tool batches`, `4743915 test(agent): preserve parallel tool safety semantics`, and `2172776 feat(ai_chat): show tool result target resources` add safe parallel dispatch, preserve approval gating, keep observation order deterministic, and show target resource labels on tool result cards. `agent_runtime` Phase 6 checks and `ai_chat_view` tool card checks passed on 2026-07-02. | Add folded target-grouped result sections and batched high-risk approval in later checkpoints. |

### Active Checkpoint

Current checkpoint: Phase 3c manual terminal-exec smoke and multi-resource result grouping follow-up.

Purpose:

1. Verify `terminal.exec` writes into the visible terminal pane with real app behavior.
2. Add folded target-grouped tool result sections for multi-resource operations.
...
```text
2172776 feat(ai_chat): show tool result target resources
```
...
rtk cargo test -p ai_chat_view tool_observation_records_target_resource_label_on_card
...
3. Add folded target-grouped result sections for multi-resource operations.
