...
| Phase 5 Resource Pool UI | In progress | Resource pool wording, default target display, search, type filtering, and default-target builder semantics are implemented in `ai_chat_view`. Phase 5b plan exists at `docs/superpowers/plans/2026-07-02-unified-tool-runtime-phase-5b-resource-pool-management.md`. | Next checkpoint: add explicit add/remove resource pool management, then workspace/tag/all source selection. |
