...
| Phase 6 Multi-resource execution | Done | `23cbec8 feat(agent): batch executable tool calls`, `29e6f01 feat(agent): dispatch parallel-safe tool batches`, `4743915 test(agent): preserve parallel tool safety semantics`, `2172776 feat(ai_chat): show tool result target resources`, and `d51e7a2 feat(ai_chat): group tool results by target` add safe parallel dispatch, preserve approval gating, keep observation order deterministic, show target resource labels on tool result cards, and fold consecutive same-target tool results under a target header. `agent_runtime` Phase 6 checks passed on 2026-07-02; `ai_chat_view` target grouping checks passed on 2026-07-03. | Add batched high-risk approval in a later checkpoint. |

### Active Checkpoint

Current checkpoint: Phase 3c manual terminal-exec smoke and batched approval follow-up.

Purpose:

1. Verify `terminal.exec` writes into the visible terminal pane with real app behavior.
2. Add batched high-risk approval for multi-resource operations.
...
```text
d51e7a2 feat(ai_chat): group tool results by target
```

Last checkpoint verification run:

```bash
rtk cargo test -p ai_chat_view tool_card
rtk cargo test -p ai_chat_view render_items_group_consecutive_tool_cards_by_target
rtk cargo test -p ai_chat_view render_items_do_not_group_tool_cards_across_text_messages
...
3. Add batched high-risk approval for multi-resource operations.
