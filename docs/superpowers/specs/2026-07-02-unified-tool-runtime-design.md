...
| Phase 5 Resource Pool UI | In progress | Resource pool wording, default target display, search, type filtering, and default-target builder semantics are implemented in `ai_chat_view`. Focused `ai_chat_view` tests and `cargo check -p ai_chat_view` passed on 2026-07-02. | Next checkpoint: add explicit add/remove resource pool management and workspace/tag/all source selection. |
