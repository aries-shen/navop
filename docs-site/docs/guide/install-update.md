...
| Windows | x86_64 | `.msi` | MSI 安装包，提供开始菜单、桌面快捷方式和稳定的文件关联 |
| Windows | x86_64 | `.exe` | EXE 安装包，封装同一套当前用户 MSI 安装流程 |
| Windows | x86_64 | `.zip` | 免安装运行；数据仍保存在 Windows 用户目录 |
| Windows | x86_64 | `-portable.zip` | 数据与程序放在同一目录、可整体移动 |
