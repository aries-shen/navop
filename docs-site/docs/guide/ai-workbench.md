...
可以新建、重命名、归档和删除对话或任务。归档用于整理，不等于安全擦除外部模型服务已经接收的数据。工具面板会展示调用、计划和审批；AI 工具调用默认采用「手动确认」模式，模型请求业务工具前需先通过确认，降低误操作风险。也可按任务调整为更宽松的档位，并阅读工具名称、目标资源、参数和潜在副作用。

审批等待时间可配置，超过等待时长后审批按超时处理并返回相应结果。确认超时前请阅读工具用途和参数；需要更长时间判断时，可在设置中调大等待时间。
