...
## X11 转发与 macOS XQuartz

SSH 连接可启用 X11 转发，将远端 X11 图形应用显示到本机 X server；该选项对新打开的终端会话生效。macOS 需要先安装并启动 XQuartz，远端服务器也必须允许 X11 Forwarding。

v0.9.3 改进了 Intel Mac 对旧版 XQuartz 环境、套接字和认证信息的检测。若启用后仍无法显示，先确认 XQuartz 正在运行，关闭并重新打开 Navop 与 SSH 会话，再检查远端 `sshd` 配置和当前账号权限。X11 环境不可用时，Navop 会停用转发，但不会阻断普通 SSH 终端。
