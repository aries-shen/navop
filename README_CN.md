...
<div align="center">
  ...
  <p>
    <img src="app1.png" alt="Navop 概览" width="820" />
  </p>
</div>

## v0.9.3 更新亮点

- **打开连接更直接** — 双击首页连接卡片或常驻连接侧边栏中的连接行，即可直接打开连接。
- **连接导航更清晰** — 优化侧边栏交互、层级和样式，并移除 macOS 上多余的标题分隔线。
- **工具布局更稳定** — 调整数据库、Redis、MongoDB 与终端工具侧栏，修复树宽度调整和面板拖动跳变。
- **远程桌面体验改进** — 修复 macOS 下 RDP 窗口标题栏与面板拖动问题。
- **Intel Mac SSH 兼容性** — 改进 Intel Mac 对旧版 XQuartz 的 X11 环境检测与兼容。
...
在同一界面连接 MySQL、PostgreSQL、SQLite、DuckDB、SQL Server、Oracle 和 ClickHouse。网络数据库连接支持每连接 SOCKS5 / HTTP CONNECT 代理、代理认证，以及“通过代理连接 SSH 再建立数据库隧道”。可浏览数据库、Schema、表、视图、字段、索引、外键、过程、函数、触发器和序列等对象，并通过 ER 图查看关系，具体能力取决于数据库类型。
...
提供 SQL 编辑、Schema 感知补全、查询执行、事务控制、执行计划和多查询页签。可查看或编辑结果集、导入导出数据、设计表结构，并通过数据库比较工具检查 Schema/Data 差异、生成同步计划和执行多表同步。AI 助手可结合当前数据库生成、解释和优化 SQL。
...
在标签页和原生可拖拽分屏中打开 SSH 或本地终端，并使用快捷命令分组、命令历史、广播输入、Shell integration 和终端 AI 侧边栏。本地终端支持系统默认 Shell、PowerShell、CMD、WSL、Git Bash 和自定义程序。SFTP 支持上传下载、远程编辑、拖拽或粘贴、远程端点搜索、路径收藏和跨服务器直接复制。可复用端口转发、服务器监控、串口连接与 RDP/VNC 远程桌面，让常用远程运维流程集中在同一应用中。终端会话还可把剪贴板图片传递给兼容的服务器端 TUI 应用。

### Agent Hub

Agent Hub 将终端 Agent、项目文件树、Git 分支、变更列表与并排 Diff 汇聚到同一个工作区，让编码、资源浏览和版本管理自然衔接。工作区可手动选择目录或跟随终端路径，并按需显示隐藏文件与 Git 忽略文件；也可搜索、新建、拉取、推送和切换本地或远程分支，在提交前对照 HEAD 逐项确认 Agent 修改。
...
| 全局导航 | 分类卡片 |
|:-:|:-:|
| [![全局导航](app1.png)](app1.png) | [![分类卡片](app.png)](app.png) |

| Agent Hub 工作区 |
|:-:|
| [![Agent Hub 工作区](agent_hub.png)](agent_hub.png) |
