...
- **修正更新版本识别** — 发布包现在会正确报告 v0.9.2，避免用户完成升级后仍反复收到同一版本的更新提示。
- **首页布局命名更清晰** — 两种首页体验分别命名为 **全局导航** 和 **分类卡片**，不再将任一选项描述为旧版或过时视图。
- **完善便携版说明** — 文档补充 Windows ZIP 包、便携数据目录、手动升级、备份、文件关联和高级便携参数的完整使用方式。
- **完善 Agent Hub 展示** — 更新截图与文档，将终端 Agent、工作区资源管理器、Git 分支、变更列表和并排 Diff 作为一体化编码工作区展示。
- **提升扩展注册稳定性** — 原生扩展发现会忽略不属于可加载原生扩展的 SQL manifest 文件。
