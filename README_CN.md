...
- **SFTP 服务器间复制** — 新增可搜索的端点切换器，可在两台远程服务器之间直接复制文件或目录，无需先落到本机中转。
- **本地终端与 AI** — 新增打开终端时选择 Profile、按平台展示可用 Shell，并让终端 AI 侧边栏支持带实时终端上下文的本地会话。
- **Notes 文档导出** — 通过沙箱化 Rust WASM 导出扩展，将文档导出为自包含 HTML、PDF 和 Word DOCX。
- **数据库工作区体验** — 新增数据库对象行右键菜单、稳定的结果区滚动，并修复 SQL 结果摘要高度与固定滚动条问题。
- **远程桌面性能** — RDP 与 VNC Provider 改为增量帧传输，提升活跃会话响应速度，同时改进 VNC 卡住后的恢复能力。
- **连接与凭据整理** — 首页工作区区分个人密钥与团队密钥，SFTP 端点标题也会明确显示当前本地或远程侧。
- **扩展、CLI 与发版可靠性** — Redis/MongoDB 原生驱动迁移为可安装扩展，补齐全局 Navop CLI/Skill 工作流，并支持跨平台 Release 构建独立补跑修复。
...
Notes 工作区支持本地 Markdown 文档、富文本与 Markdown 双向转换、白板 Markdown bundle、语法高亮、Mermaid 和数学公式渲染。文档位置、编辑器快捷键和 AI Provider 均可配置；渲染能力还可以通过扩展提供和更新，沙箱化 WASM 导出器还可生成自包含 HTML、PDF 和 Word DOCX 文件。
