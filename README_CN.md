...
- **更完整的 Notes Markdown 渲染** — 文档视图支持标准 Markdown 语法、受限 HTML 安全渲染和 Markdown 表格内图片展示；相对媒体资源按当前文档目录解析，白板转换则生成便于分享的预览。
- **更快的语法高亮** — 按语言复用高亮器，修正全文更新的位置计算，并保持语言 WASM 解析器按需加载。
- **更流畅的扩展管理** — 安装、重载和卸载只刷新受影响的扩展类型；卸载在后台执行并展示进度，避免扩展管理界面卡顿。
- **Redis 驱动按需安装** — Navop 会从已安装扩展目录动态发现驱动，缺少 Redis 驱动时提示安装，并在完成后直接继续打开连接。
- **Release 产物恢复** — 发版流程可处理尚无 Release 或 Release 中没有历史产物的情况，并支持使用现有构建产物直接修复发布内容。
