...
/// Optional observer for the terminal lock acquired by the PTY parser.
///
/// The callback is invoked once per acquisition after the guard is released.
/// It is intentionally generic so embedders can connect their own metrics
/// without introducing a dependency from `alacritty_terminal`.
pub type TerminalLockObserver = Arc<dyn Fn(Duration, Duration) + Send + Sync>;
...
pub struct EventLoop<T: tty::EventedPty, U: EventListener> {
    ...
    drain_on_exit: bool,
    ref_test: bool,
    terminal_lock_observer: Option<TerminalLockObserver>,
}
    ...
    U: EventListener + Send + 'static,
{
    ...
    pub fn new(
        ...
        Self::new_with_terminal_lock_observer(
            terminal,
            event_proxy,
            pty,
            drain_on_exit,
    ...
    /// Create a new event loop with optional terminal lock instrumentation.
    pub fn new_with_terminal_lock_observer(
        terminal: Arc<FairMutex<Term<U>>>,
        event_proxy: U,
        pty: T,
        drain_on_exit: bool,
        ...
        X: Write,
    {
        ...
        loop {
            ...
            let terminal = match &mut terminal {
                ...
                None => {
                    if self.terminal_lock_observer.is_some() && lock_wait_started.is_none() {
                        lock_wait_started = Some(Instant::now());
                    }
                    ...
                    if self.terminal_lock_observer.is_some() {
        ...
        drop(terminal);
        if let (Some(observer), Some(wait_started), Some(acquired_at)) = (
            &self.terminal_lock_observer,
            lock_wait_started,
            lock_acquired_at,
        ) {
            observer(
                acquired_at.saturating_duration_since(wait_started),
                Instant::now().saturating_duration_since(acquired_at),
            );
        }
